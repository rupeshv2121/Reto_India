...
const Review = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    if (!text.Reviews.trim()) {
      toast.error('Please enter a review before submitting.', { /* Toast options */ });
      return;
    }
    if (text.Rating === 0) {
      toast.error('Please select a rating before submitting.', { /* Toast options */ });
      return;
    }
    ...
    try {
      ...
      toast.success('Review submitted successfully.', { /* Toast options */ });
      setText({ name: 'Customer name', Rating: 0, Reviews: '', img: '' });
      fetchData(); // Refresh the reviews
    } catch (error) {
      console.error('Error submitting review:', error);
      toast.error('Failed to submit the review.', { /* Toast options */ });
    }
  };
  ...
  return (
    <div className='ReviewSection ' >
      ...
      <div className='ReviewText'>
        ...
        <div>
          <form onSubmit={handleSubmit} className='TextAreaBox w-full' encType="multipart/form-data">
            <textarea
              id="message"
              rows="4"
              value={text.Reviews}
              ...
              <div className='upper_text'>
                ...
                <div className='rating_img w-4 h-4'>
                    {[...Array(5)].map((_, index) => (
                      <StarRateIcon
                        key={index}
                        className={`RateStar ${index < text.Rating ? 'text-yellow-400' : 'text-gray-400'}`}
            ...
            <div className='UploadBox flex flex-col justify-center w-full'>
              ...
              <label className="block mb-2  text-gray-900" htmlFor="file_input">Upload Image</label>
              <div className='w-full flex justify-center '>
                <input className="block w-4/5 text-sm my-2 text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 dark:text-gray-400 focus:outline-none dark:bg-gray-700 dark:border-gray-600 dark:placeholder-gray-400" id="file_input" type="file"/>
              </div>
        ...
        <div className='ReviewCarousel'>
          <div className='text-center'><h1>All reviews</h1></div>
